/*
  Id.Programa: G4-Ej05 Articulos ABM stream.Cpp
...
void ObtClv(short &clvCodArt, short x, short y) {
  ...
  do {
    ...
    clreol();
    cin >> clvCodArt;
  } while (!(clvCodArt >= 0 && clvCodArt <= 100));
}  // ObtClv

bool Existe(fstream &Art, short clvCArt, sArt &rArt) {
  Art.seekg(clvCArt * sizeof rArt, ios::beg);  // cur, end
...
void Listar(fstream &Art, short x, short y) {
  ...
  for (short i = clvCodArtDsd; i <= clvCodArtHst; i++) {
    Art.read((char *)&rArt, sizeof rArt);
    if (rArt.CodArt) {
      nroOrd++;
