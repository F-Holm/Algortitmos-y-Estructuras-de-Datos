/*
  Id.Programa: LeerIdxArt_Descrip_R2-P1.cpp
...
void Abrir(ifstream &ArtIdx) {
  ArtIdx.open("IdxDescrip.Idx", ios::binary);
} // Abrir
...
int _tmain(int argc, _TCHAR* argv[]) {
  ...
  Abrir(ArtIdx);
  cout << "Listado de Articulos ordenado por Descripcion" << endl;
  cout << "Descripcion         Ref.Art." << endl;
  cout << "----------------------------" << endl;
  while (ArtIdx.read((char *) &rIdx, sizeof rIdx))
    cout << rIdx.descrip << ' ' << setw(3) << rIdx.refArt << endl;
  cout << "----------------------------" << endl;
  ArtIdx.close();
  system("pause");
  return 0;
