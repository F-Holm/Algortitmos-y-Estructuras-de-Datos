/**************
*	Id. Programa: G5Ej04jArcCascaron.Cpp
...
*                 Archivo cascaron con 365 componentes,
*                 una por cada día del año, de Imp.Acum.
***************/
...
void AcumGto(FILE *AGtos, sGasto rGto) {
  ...
  posAGtos = CantDias(rGto.Mes) + rGto.Dia - 1;
  fseek(AGtos,posAGtos * sizeof (float),SEEK_SET);
  readf(AGtos,impAcum);
  impAcum += rGto.Impo;
  fseek(AGtos,posAGtos * sizeof(float),SEEK_SET);
  writef(AGtos,impAcum);
...
void Listado(FILE *AGtos) {
	...
	freopen("LstGtosMDj.Lst","w",stdout);
	cout << "(j) Listado de Gastos por Mes y Dia Acumulado (j)" << endl << endl;
	cout.setf(ios::fixed);
	cout.precision(2);
	rewind(AGtos);
