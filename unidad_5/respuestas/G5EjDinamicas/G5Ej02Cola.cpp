...
void Listado(tmCola mC) {
  ...
  cout.precision(2);
  cout.setf(ios::fixed);
  cout << "Listado de Gastos ordenados por mes y en mismo orden de importes de lectura" << endl;
  for (register i = 1; i <= MESES; i++) {
    cout << "Mes: " << setw(2) << i << " ";
    cout.setf(ios::left);
...
int main() {
  FILE   *Gastos;
  sNodo  *mColas[MESES + 1][2];
  sGasto rGasto;

  Abrir(&Gastos);
  InicTbl(mColas);
  while (fread(&rGasto, sizeof(rGasto), 1, Gastos))
    Enqueue(mColas[rGasto.mes][0], mColas[rGasto.mes][1], rGasto.importe);
  Listado(mColas);
  fclose(Gastos);
